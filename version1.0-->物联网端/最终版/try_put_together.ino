...
#define WAITTIME 800
#define PubSucc "\"code\":200,\"msg\":\"success\"}"
...
char *ClientID= "CO2_READ2";          //设备ID-存放客户端ID的缓冲区- 1035997083
...
char *Passward= "version=2018-10-31&res=products%2FTKDr9E8tk0%2Fdevices%2FCO2_READ2&et=3020404378&method=md5&sign=%2BeAfiL9Ux7M2RsURFkJBGA%3D%3D";         //鉴权信息-存放密码的缓冲区-JianQuanXX 
...
char *topicPostPub = "$sys/TKDr9E8tk0/CO2_READ2/thing/property/post";       //topic=>PUB                                                                     //订阅
char *topicPostSub = "$sys/TKDr9E8tk0/CO2_READ2/thing/property/post/reply";  //订阅是要求平台操作或回应，topic，Onenet数据点上传topic（不用改）
char *topicSetSub = "$sys/TKDr9E8tk0/CO2_READ/thing/property/set";          //订阅
char *topicSetPub = "$sys/TKDr9E8tk0/CO2_READ2/thing/property/set_reply";    //topic，Onenet数据点上传topic（不用改）
...
//---------------------------------------------------------------------
    ...
    MQTT_Subscribe(topicSetSub, 0);     // QoS=0/1都行
    MQTT_TXData(RXTX_BUF,Fixed_len + Variable_len + Payload_len);//回显和发送
    //Serial.println("订阅回复：");  
    MQTT_WaitBK("\x90\x03",100);       //连接回复OK"\x90\x03"=RXTX_BUF[0~1]
    
    Serial.println("发(s),再在OneNet发命令");
...
void loop() {
    ...
    if(Serial.available()){
      tst = (char)Serial.read();
      Serial.println("设备调试下发命令");
      while(Serial1.read()>=0){}

    }
    if ((tst=='s')&&Serial1.available()) 
    {
      //等待服务器的操作指令：
      //Serial.print("格式见前");
      Serial.println(tst);
      MQTT_WaitPUB("\"fan\":",true);
      delay(500);
      
      MQTT_TXData(RXTX_BUF,Fixed_len + Variable_len + Payload_len);//发送
    }
...
int readCO2(){
  ...
  //读CO2浓度代码
   ...
   { 
      Serial2.readBytes(buffer, 18);  // 读取6个字节到数组中
      if(buffer[0] == 0x2C)
      {
...
char MQTT_WaitPUB(char *flag,int timeout)
{   
  int tmp;
  char id[128];
  char *pubStr=id;
  ...
  while(timeout--){                    //等待超时时间到0
    ...
    Serial.print("提取灯前");
    Serial.println(WiFi_RxCounter);
    Serial.println(RXTX_BUF[0]);
    if((WiFi_RxCounter>0)&&(RXTX_BUF[0]=='\x30'))  //为PUB，平台发给终端
    {
      if(RXTX_BUF[1]<0x80)  //假设长度占用一字节
      {
        tmp=RXTX_BUF[1]-(RXTX_BUF[2]*256+RXTX_BUF[3]);
        if(tmp>0)   //说明有PayLoad
        {
          result=strstr(&RXTX_BUF[3+(RXTX_BUF[2]*256+RXTX_BUF[3])],flag);
          Serial.print("提取灯的结果：");
          Serial.println(result);
          if(result)
          {
            //发送数值
            if((result[strlen(flag)])=='1')    //':'后面-变亮
            {
              //发送开关量
              
              digitalWrite(fanPin,0);
            }
            else if((result[strlen(flag)])=='0')//':'后面-变灭
            {
              //发送开关量
              digitalWrite(fanPin,1);
              delay(5000);
          ...
          result=strstr(&RXTX_BUF[3+(RXTX_BUF[2]*256+RXTX_BUF[3])],"{\"id\":");//3为了避开4个字节再加可变头，即为负载
          Dou=strstr(result,",");
          memcpy(&result[Dou-result+1],PubSucc,strlen(PubSucc));//+1,从‘，’后替代
          //回复set,即set_reply
          result[Dou-result+1+strlen(PubSucc)]='\0';
          //Serial.print("回复结果："); 
          //Serial.println(result);
          //for(int i=0;i<WiFi_RxCounter;i++)
          //  Serial.print(RXTX_BUF[i]);       //串口输出信息
          //Serial.println();
          memcpy(pubStr,result,strlen(result));
          pubStr[strlen(result)]='\0';
          MQTT_PublishQos(topicSetPub, pubStr, strlen(pubStr));
          break;
        }
      }
      else    //假设长度占用两字节
  ...
  //Serial.print("\r\nresponse:");     //表示成功，否则不成功
  for(int i=0;i<Fixed_len + Variable_len + Payload_len;i++)
      Serial.print(RXTX_BUF[i]);       //串口输出信息
  Serial.println();
  if(timeout <= 0)return 1;            //如果timeout<=0，说明超时时间到了，也没能收到WIFI GOT IP，返回1
  return 0;                            //正确，返回0
