...
void setup() {
  ...
  Serial.begin(9600);
  mySerial.begin(9600);//与ESP8266通信
  while(!Serial){}
  //同步等待Serial初始化成功
  ...
  if(WiFi_Connect_IoTServer()==0){
    ...
    //  t = analogRead(0)+4000;
     ...
     if( (t / 1000) >= 1 ){
       ...
       dataCMD[strlen(dataCMD)-7] = t / 1000 + ' ';
     }
     dataCMD[strlen(dataCMD)-6] = (t / 100) % 10 + '0';
     dataCMD[strlen(dataCMD)-5] = (t / 10) % 10 + '0';
     dataCMD[strlen(dataCMD)-4] = t % 10 + '0';
...
int WiFi_Reset(int timeout){
  ...
  mySerial.print(RXTX_BUF);//发送指令
  memset(RXTX_BUF,0,RXTXBUF_SIZE);//清空WiFi接收缓冲区
  while(timeout--){
    delay(50);
    while(mySerial.available()){
      RXTX_BUF[WiFi_RxCounter] = (char)mySerial.read();
